...
def get_activations_from_dset(
    dset, model, batch_size=50, dims=2048, device='cpu', num_workers=1):
...
def get_activations(files, model, batch_size=50, dims=2048, device='cpu',
    ...
    dataset = ImagePathDataset(files, transforms=val_transform)
...
def calculate_activation_statistics(files, model, batch_size=50, dims=2048,
                                    device='cpu', num_workers=1, return_acts=False):
    """Calculation of the statistics used by the FID.
    Params:
    -- files       : List of image files paths OR a dataset object
    -- model       : Instance of inception model
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the inception model.
    """
    if isinstance(files, torch.utils.data.Dataset):
        act = get_activations_from_dset(files, model, batch_size, dims, device, num_workers)
    else:
        act = get_activations(files, model, batch_size, dims, device, num_workers)
    
    act_np = act.cpu().numpy()
    mu = np.mean(act_np, axis=0)
    sigma = np.cov(act_np, rowvar=False)
